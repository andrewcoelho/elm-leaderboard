...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var helper = F3(function (lst,elem,offset) {
      helper: while (true) {
         var _p0 = lst;
         if (_p0.ctor === "[]") {
               return -1;
            } else {
               if (_U.eq(_p0._0,elem)) return offset; else {
                     var _v1 = _p0._1,_v2 = elem,_v3 = offset + 1;
                     lst = _v1;
                     elem = _v2;
                     offset = _v3;
                     continue helper;
   ...
   var indexOf = F2(function (lst,element) {    return A3(helper,lst,element,0);});
   var update = F2(function (action,model) {
      var _p1 = action;
      if (_p1.ctor === "Select") {
            return _U.update(model,{selected: _p1._0});
         } else {
            var updatePlayer = function (p) {    return _U.eq(p.id,_p1._0) ? _U.update(p,{points: p.points + 5}) : p;};
            return _U.update(model,{players: A2($List.map,updatePlayer,model.players)});
         }
   });
   var Increment = function (a) {    return {ctor: "Increment",_0: a};};
   var controls = F2(function (address,selected) {
      return A2($Html.footer,
      _U.list([]),
      _U.list([A2($Html.span,_U.list([]),_U.list([$Html.text(selected.name)]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Increment(selected.id))]),_U.list([$Html.text("Add 5 Points")]))]));
   });
   var Select = function (a) {    return {ctor: "Select",_0: a};};
   var playerItem = F3(function (address,lst,player) {
      return A2($Html.li,
      _U.list([A2($Html$Events.onClick,address,Select(player))]),
      _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("position")]),_U.list([$Html.text($Basics.toString(A2(indexOf,lst,player) + 1))]))
              ,A2($Html.span,_U.list([$Html$Attributes.$class("name")]),_U.list([$Html.text(player.name)]))
              ,A2($Html.span,_U.list([$Html$Attributes.$class("points")]),_U.list([$Html.text($Basics.toString(player.points))]))]));
   });
   var view = F2(function (address,model) {
      var sortedList = A2($List.sortBy,function (_) {    return _.points;},model.players);
      var reversedList = $List.reverse(sortedList);
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("container")]),
      _U.list([A2($Html.header,_U.list([]),_U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Leaderboard")]))]))
              ,A2($Html.ul,_U.list([]),A2($List.map,A2(playerItem,address,reversedList),reversedList))
              ,A2(controls,address,model.selected)]));
   });
   var model = {players: _U.list([{name: "Nikola Tesla",points: 0,id: 0}
                                 ,{name: "Grace Hopper",points: 0,id: 1}
                                 ,{name: "Ada Lovelace",points: 0,id: 2}
                                 ,{name: "Albert Einstein",points: 0,id: 3}
                                 ,{name: "Carl Friedrich Gauss",points: 0,id: 4}
                                 ,{name: "Marie Curie",points: 0,id: 5}
                                 ,{name: "Claude Shannon",points: 0,id: 6}])
               ,selected: {name: "Claude Shannon",points: 0,id: 6}};
   var app = $StartApp$Simple.start({model: model,update: update,view: view});
   var main = app;
   var Player = F3(function (a,b,c) {    return {name: a,points: b,id: c};});
   var Model = F2(function (a,b) {    return {players: a,selected: b};});
